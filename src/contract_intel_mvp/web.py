...
DISCOVERY_SYSTEM_PROMPT = (
    ...
    "what step you're on by READING the current_signature you receive in the "
    ...
    "STEP DETECTION (read current_signature CAREFULLY before answering):\n"
    "  - If target_class is empty → you are at Step 1.\n"
    "  - If target_class is set BUT target_description is empty → Step 2.\n"
    "  - If both are set BUT clause_types is empty → Step 3 (propose definition).\n"
    "  - If clause_types contains only must-have items → Step 4 done, ask Step 5 "
    "(disqualifying clauses).\n"
    "  - If clause_types has both must-have and must-not-have items → Step 6 "
    "(recap).\n"
    "  - The user's most recent message tells you whether the previous step was "
    "confirmed (yes / acknowledgement) or needs refinement (no / pushback).\n\n"
    "  NEVER re-ask a question whose answer is already stored. NEVER regress to "
    "an earlier step.\n\n"
    "STEPS:\n"
